...
export const DEFAULT_SETTINGS: RestaurantSettings = {
  ...
  businessCloseTime: "02:00",
  autoDayCloseEnabled: true,
};
...
// Watches the local clock; when it crosses `businessCloseTime`, writes
// a one-shot audit log entry marking the previous business day as
// settled and dispatches an update so all "today" widgets recompute.
// Unpaid orders carry forward (no data is deleted).
const DAY_CLOSE_KEY = "spices_last_day_close";
function applyDayCloseSchedule() {
  try {
    const settings = read<RestaurantSettings>(KEYS.settings, DEFAULT_SETTINGS);
    if (settings.autoDayCloseEnabled === false) return;
    const now = Date.now();
    const currentDayStart = getBusinessDayStart(now, settings);
    const last = Number(localStorage.getItem(DAY_CLOSE_KEY) || 0);
    // Only fire once per business day, and only after the close has passed
    // (i.e. we are already inside a new business day whose start > last).
    if (currentDayStart > last && last > 0) {
      const orders = read<Order[]>(KEYS.orders, []);
      const dayOrders = orders.filter(
        (o) => o.createdAt >= last && o.createdAt < currentDayStart
      );
      ...
      Store.addAudit({
        ...
        details: `Auto-settled business day. Orders: ${dayOrders.length}, Sales: ₹${totalSales.toFixed(
          2
        )}, Unpaid carried forward: ${unpaid}`,
      });
      console.log(
        `[Store] Business day auto-closed. Orders=${dayOrders.length} Sales=${totalSales} Unpaid=${unpaid}`
      );
      dispatchUpdate("all");
    }
    localStorage.setItem(DAY_CLOSE_KEY, String(currentDayStart));
  } catch (e) {
    console.error("[Store] applyDayCloseSchedule failed", e);
  }
}

if (typeof window !== "undefined") {
  // Recheck schedules every 30s.
  setInterval(applyMenuProfileSchedule, 60_000);
  setInterval(applyDayCloseSchedule, 30_000);
  // Prime the checkpoint on first run so we don't emit a spurious log.
  setTimeout(applyDayCloseSchedule, 2_000);
  // Also re-check when the tab regains focus / becomes visible, so the
  // 02:00 auto-reset fires even if the machine was asleep.
  window.addEventListener("focus", applyDayCloseSchedule);
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") applyDayCloseSchedule();
  });
}
...
export const Store = {
  ...
  /**
   * Reconcile the BASE menu (categories + items) against a canonical spec.
   *  - Categories/items are matched case-insensitively by name.
   *  - Missing entries are CREATED, matching entries are UPDATED
   *    (price / veg / category / isBengali), extras are DELETED.
   *  - Menu Profiles (festival menus) are left untouched.
   */
  syncMenuFromSpec: (
    spec: { name: string; items: { name: string; price: number; veg: boolean; isBengali?: boolean }[] }[]
  ): {
    catsCreated: number;
    catsDeleted: number;
    itemsCreated: number;
    ...
    // First, run dedupe so we start from a clean base.
    Store.dedupeBaseCategories();
    const norm = (s: string) => s.trim().toLowerCase();

    const existingCats = read<MenuCategory[]>(KEYS.categories, []);
    const existingItems = read<MenuItem[]>(KEYS.items, []);

    const catByName = new Map<string, MenuCategory>();
    existingCats.forEach((c) => catByName.set(norm(c.name), c));

    // Build final category list in spec order, reusing existing ids.
    const specNames = new Set(spec.map((s) => norm(s.name)));
    let catsCreated = 0;
    const finalCats: MenuCategory[] = spec.map((s, i) => {
      const existing = catByName.get(norm(s.name));
      if (existing) return { ...existing, name: s.name, order: i };
      catsCreated++;
      return { id: uid("cat"), name: s.name, order: i };
    });
    const catsDeleted = existingCats.filter((c) => !specNames.has(norm(c.name))).length;
    const catIdByName = new Map<string, string>();
    finalCats.forEach((c) => catIdByName.set(norm(c.name), c.id));

    // Reconcile items scoped per category (item names can repeat across cats).
    ...
    for (const specCat of spec) {
      const catId = catIdByName.get(norm(specCat.name))!;
      const existingInCat = itemsByCat.get(catId) ?? [];
      const byName = new Map<string, MenuItem>();
      existingInCat.forEach((i) => byName.set(norm(i.name), i));

      for (const specItem of specCat.items) {
        const key = norm(specItem.name);
        ...
        if (existing) {
          const next: MenuItem = {
            ...
            isBengali: specItem.isBengali,
          };
          if (
            existing.name !== next.name ||
            existing.price !== next.price ||
            existing.veg !== next.veg ||
            existing.categoryId !== next.categoryId ||
            (existing.isBengali ?? false) !== (next.isBengali ?? false)
          ) {
            itemsUpdated++;
          }
    ...
    const itemsDeleted = existingItems.length - keptIds.size;

    write(KEYS.categories, finalCats);
    write(KEYS.items, finalItems);
    return { catsCreated, catsDeleted, itemsCreated, itemsUpdated, itemsDeleted };
  },
  ...
  appendItemsToOrder: (
    ...
    newItems.forEach((ni) => {
      ...
      const existingIdx = merged.findIndex(
        (m) =>
          m.menuItemId === incoming.menuItemId &&
          m.kotPrinted !== true &&
          m.printed !== true
      );
      if (existingIdx >= 0) {
  ...
  savePayments: (list: Payment[]) => write(KEYS.payments, list),
  addPayment: (p: Payment) => {
    write(KEYS.payments, [...Store.listPayments(), p]);
  },
  /**
   * Replace ALL payment records for a given order with a fresh set derived
   * from a cash/UPI split. Also updates the order's `lastPaymentMode` so
   * downstream reports (which fall back to lastPaymentMode for legacy
   * orders) stay consistent. Used by Reports > Item-wise Sales edit flow.
   */
  replaceOrderPayments: (
    orderId: string,
    args: { cashAmount: number; upiAmount: number; receivedBy: string; reference?: string }
  ) => {
    const order = Store.listOrders().find((o) => o.id === orderId);
    if (!order) return;
    const cash = Math.max(0, Number(args.cashAmount) || 0);
    const upi = Math.max(0, Number(args.upiAmount) || 0);
    const total = cash + upi;
    const mode: Payment["paymentMode"] =
      cash > 0 && upi > 0 ? "part_payment" : upi > 0 ? "upi" : "cash";
    const rec: Payment = {
      id: uid("pay"),
      orderId,
      ...
      upiAmount: upi,
      totalPaid: total,
      balanceDue: Math.max(0, (order.grandTotal || 0) - total),
      receivedBy: args.receivedBy,
      createdAt: Date.now(),
      reference: args.reference,
    };
    const kept = Store.listPayments().filter((p) => p.orderId !== orderId);
    Store.savePayments([...kept, rec]);
  ...
  // Manually close the current business day right now. Sets a floor so
  // Dashboard/Reports "today" counters immediately reset to zero, and
  // writes an audit log entry. Does not delete any data.
  closeBusinessDayNow: (actor?: { id: string; name: string }) => {
    try {
      const now = Date.now();
      const settings = read<RestaurantSettings>(KEYS.settings, DEFAULT_SETTINGS);
      const prevStart = getBusinessDayStart(now, settings);
      const orders = read<Order[]>(KEYS.orders, []);
      const dayOrders = orders.filter((o) => o.createdAt >= prevStart && o.createdAt < now);
      const totalSales = dayOrders
        .filter((o) => o.status === "paid" || o.status === "billed")
        .reduce((s, o) => s + o.grandTotal, 0);
      const unpaid = dayOrders.filter(
      ...
      localStorage.setItem("spices_manual_day_close", String(now));
      localStorage.setItem(DAY_CLOSE_KEY, String(now));
      Store.addAudit({
        userId: actor?.id ?? "system",
        userName: actor?.name ?? "System",
        action: "BUSINESS_DAY_CLOSED_MANUAL",
        details: `Manual business-day close. Orders: ${dayOrders.length}, Sales: ₹${totalSales.toFixed(
          2
        )}, Unpaid carried forward: ${unpaid}`,
      });
      dispatchUpdate("all");
      console.log("[Store] Manual business-day close fired at", new Date(now).toISOString());
    } catch (e) {
      console.error("[Store] closeBusinessDayNow failed", e);
    }
  },
