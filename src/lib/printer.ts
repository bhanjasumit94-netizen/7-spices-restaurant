...
type PrintType = "bill" | "kot" | "reprint" | "kot_additional" | "kot_final";

// Detect a mobile/tablet browser. On mobile we never call window.print()
// automatically — auto-print is blocked or surfaces a tiny share-sheet
// instead of the printer. We surface a large in-page "Tap to Print"
// button so the user explicitly fires the print dialog.
export function isMobilePrintDevice(): boolean {
  ...
  // iPadOS 13+ reports as Mac — use touch + coarse pointer as a tiebreaker.
  try {
...
function renderKotBody(
  ...
  settings: RestaurantSettings,
  copyLabel: string,
  variant: "initial" | "additional" | "final" = "initial",
) {
  ...
  const footerTitle =
    variant === "additional"
      ? "*** ADDITIONAL KOT ***"
      : variant === "final"
      ? "*** FINAL KOT — KITCHEN VERIFICATION ***"
      : "*** KOT ***";
  ...
  return `
    ...
    ">
      ...
      <div class="center kot-header">${escapeHtml(headerTitle)}</div>
      ${copyLabel ? `<div class="center copy-tag">${escapeHtml(copyLabel)}</div>` : ""}
      <div class="dline"></div>
      ...
      <div class="kv kot-row"><span>Time</span><b>${fmtTimeOnly(Date.now())}</b></div>
      ...
      <div class="dline"></div>
      <div class="center kot-footer">${escapeHtml(footerTitle)}</div>
    </div>
  `;
}
...
function buildDocument(
  ...
  const isKotType = type === "kot" || type === "kot_additional" || type === "kot_final";
  const defaultFont = isKotType ? getKotFontScale() : getBillFontScale();
  ...
  if (isKotType) {
    const variant: "initial" | "additional" | "final" =
      type === "kot_additional" ? "additional" : type === "kot_final" ? "final" : "initial";
    const showDup = settings.printDuplicateKOT === true;
    bodies.push(renderKotBody(order, settings, showDup ? "KITCHEN COPY" : "", variant));
    if (showDup) bodies.push(renderKotBody(order, settings, "DUPLICATE COPY", variant));
  } else {
    const wantCustomer = settings.printCustomerCopy !== false;
    const wantMerchant = settings.printMerchantCopy === true;
    if (wantCustomer) bodies.push(renderBillBody(order, settings, wantMerchant ? "CUSTOMER COPY" : "", type as "bill" | "reprint"));
    if (wantMerchant) bodies.push(renderBillBody(order, settings, "MERCHANT COPY", type as "bill" | "reprint"));
    if (bodies.length === 0) bodies.push(renderBillBody(order, settings, "", type as "bill" | "reprint"));
  }
...
export function printBill(
  ...
  printHtml(buildDocument(order, settings, type), { label: labelForType(type) });
}

function labelForType(type: PrintType): string {
  if (type === "kot" || type === "kot_additional") return "Print KOT";
  if (type === "kot_final") return "Print Final KOT";
  return "Print Bill";
}

// Print an additional KOT containing only the newly-added items. Caller
// passes the order plus the subset of items to print so the bill number,
// table number, waiter etc. remain consistent with the parent order.
export function printAdditionalKot(
  order: Order,
  newItems: OrderItem[],
  settings: RestaurantSettings,
) {
  if (newItems.length === 0) return;
  const subset: Order = { ...order, items: newItems };
  printBill(subset, settings, "kot_additional");
}

// Print a verification KOT with ALL items in the order (old + new).
export function printFinalKot(order: Order, settings: RestaurantSettings) {
  printBill(order, settings, "kot_final");
}
...
function printHtml(html: string, opts?: { label?: string }) {
  ...
  if (mobile) {
    // Mobile browsers block / silently fail window.print() unless the
    // call originates from a user gesture. Show a full-width sticky
    // button overlay; tap it to fire the print dialog.
    ...
    overlay.innerHTML = `
      <div style="color:#fff;font:600 14px system-ui,-apple-system,sans-serif;text-align:center">Tap ${escapeHtml(label)} to continue.</div>
      <button data-print-go style="background:#f59e0b;color:#1a1a1a;border:0;border-radius:12px;padding:18px 16px;font:800 18px system-ui,-apple-system,sans-serif;width:100%">${escapeHtml(label)}</button>
      <button data-print-cancel style="background:transparent;color:#fff;border:1px solid rgba(255,255,255,.3);border-radius:12px;padding:12px 16px;font:600 14px system-ui,-apple-system,sans-serif;width:100%">Cancel</button>
    `;
    ...
    overlay.querySelector<HTMLButtonElement>("[data-print-go]")?.addEventListener("click", () => {
      triggerPrint();
      close();
    });
    overlay.querySelector<HTMLButtonElement>("[data-print-cancel]")?.addEventListener("click", close);
    return;
  }
  ...
  setTimeout(() => {
    try {
      ...
      // Pop-up blocker or other failure — show the manual button as a fallback.
      // eslint-disable-next-line no-console
      console.warn("[print] auto-print failed, showing manual button");
    }
    cleanup();
  }, 200);
}
