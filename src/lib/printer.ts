...
import { Order, RestaurantSettings, KotSize, PrinterPaper } from "./types";
...
const PRINT_FONT_SCALE_KEY = "printFontScale";
const FONT_MIN = 0.8;
const FONT_MAX = 1.4;
...
export function getPrintFontScale(): number {
  try {
    ...
    if (raw) {
      ...
      if (!isNaN(n)) return Math.min(FONT_MAX, Math.max(FONT_MIN, n));
    }
  } catch { /* ignore */ }
  return 1;
}
export function setPrintFontScale(v: number) {
  const clamped = Math.min(FONT_MAX, Math.max(FONT_MIN, +v.toFixed(2)));
  try { localStorage.setItem(PRINT_FONT_SCALE_KEY, String(clamped)); } catch { /* ignore */ }
}
...
function buildStyles(size: PrinterPaper, printScale: PrintScale, fontScale: number) {
  ...
  const sv = printScaleValue(printScale);
  const fs = (n: number) => +(n * fontScale).toFixed(2);
  ...
  return `
    ...
    .kot .kot-name { font-size: calc(var(--kot-name) * ${fontScale}); font-weight: 800; }
    .kot .kot-header { font-size: calc(var(--kot-header) * ${fontScale}); font-weight: 800; letter-spacing: 1px; margin-top: 2px; }
    .kot .copy-tag { font-size: ${fs(14)}px; font-weight: 700; letter-spacing: 1px; margin-top: 2px; }
    .kot .kot-row { font-size: calc(var(--kot-label) * ${fontScale}); font-weight: 600; line-height: 1.4; }
    .kot .kot-table { font-size: calc(var(--kot-table) * ${fontScale}); font-weight: 800; line-height: 1.3; padding: 4px 0; }
    ...
    .kot .kot-item-name { font-size: calc(var(--kot-item) * ${fontScale}); font-weight: 800; flex: 1; text-transform: uppercase; }
    .kot .kot-qty { font-size: calc(var(--kot-qty) * ${fontScale}); font-weight: 900; }
...
function buildDocument(
  ...
  const fontScale = opts?.fontScale ?? getPrintFontScale();
...
function sampleOrder(settings: RestaurantSettings): Order {
  ...
  const items = [
    { id: "s1", itemId: "s1", name: "Steamed Rice", price: 110, quantity: 2 },
    { id: "s2", itemId: "s2", name: "Yellow Dal Fry", price: 150, quantity: 1 },
    { id: "s3", itemId: "s3", name: "Mix Veg", price: 260, quantity: 1 },
  ];
