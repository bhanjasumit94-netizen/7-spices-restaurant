...
export interface MenuCategory {
  ...
  name: string;
  order: number;
  // Profile-scoped: hide a category from POS/QR/Waiter while the
  // profile is active, without deleting it. Ignored on the base menu.
  hidden?: boolean;
}
...
export interface OrderItem {
  ...
  // Per-item KOT state. Existing busy-table items are true; newly added
  // items stay false until their Additional KOT is printed.
  kotPrinted?: boolean;
  // How many units of this line have already been sent to the kitchen.
  // Pending qty for the "New Items KOT" = quantity - (kotPrintedQty ?? 0).
  // Lets us keep a single line per item while still printing only the delta.
  kotPrintedQty?: number;
  // Legacy alias kept for older records/UI paths.
  printed?: boolean;
...
// KOT print history record. Each press of the KOT button on a busy-table
// order appends one of these so the kitchen / cashier can reprint a
// specific batch. `kind` distinguishes the original "initial" run, an
// "additional" run printed after items were appended, and the catch-all
// "final" verification print.
export interface KotRecord {
  id: string;
  ts: number;
  kind: "initial" | "additional" | "final";
  itemIds: string[];          // OrderItem.id values included in this print
  waiterId?: string;
  waiterName?: string;
  acknowledged?: boolean;     // kitchen-side ack flag
}
...
export interface Order {
  ...
  eventLog?: OrderEvent[];
  // ── KOT history ─────────────────────────────────────────────────
  // One record per KOT print (initial / additional / final). Used so
  // we can reprint the last additional KOT and so the kitchen knows
  // which items belong to which batch.
  kotHistory?: KotRecord[];
  status:
...
export type PrinterPaper = "58mm" | "80mm" | "a4";
export type KotSize = "normal" | "large" | "xlarge";
...
export interface RestaurantSettings {
  ...
  printerSize: PrinterPaper;
  kotSize?: KotSize;
  waiterMode: boolean;
  currency: string;
  logoDataUrl?: string;
  thankYouMessage: string;
  // Print automation & copies
  autoPrintKOT?: boolean;
  autoPrintBill?: boolean;
  printCustomerCopy?: boolean;
  printMerchantCopy?: boolean;
  printDuplicateKOT?: boolean;
  printLogo?: boolean;
  printGstNumber?: boolean;
  printQrCode?: boolean;
  // Watermark on printed bill (not on KOT)
  watermarkType?: "text" | "logo" | "none";
  // Optional footer extras
  facebookUrl?: string;
  instagramUrl?: string;
  websiteUrl?: string;
  tagline?: string;
  // Optional service charge % shown on the bill (display + grand total contribution)
  serviceChargePercent?: number;
  // Business day window (24h "HH:MM"). Reports/Dashboard "today"
  // counters reset when the local clock crosses businessCloseTime.
  businessOpenTime?: string;   // e.g. "11:00"
  businessCloseTime?: string;  // e.g. "02:00" (interpreted as next-day)
  autoDayCloseEnabled?: boolean;
}
...
// Advanced Seasonal Menu Profiles. Each profile carries its own copy of
// categories + items so activating a profile switches the entire POS /
// Billing / QR / KOT menu in one shot. Historical orders are unaffected
// because OrderItem stores the price at the time of billing.
export interface MenuProfile {
  ...
  startDate?: number;
  endDate?: number;
  // When true, scheduler auto-activates at startDate.
  autoActivate?: boolean;
  // When true, scheduler auto-deactivates at endDate (default true).
  autoRevert?: boolean;
  // Archived profiles are hidden from default lists and never auto-activate.
  archived?: boolean;
  createdBy: string;
