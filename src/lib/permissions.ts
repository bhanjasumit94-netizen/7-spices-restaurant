...
export type Capability =
  ...
  | "process_payments"
  | "view_menu_profiles"
  | "create_menu_profiles"
  | "activate_menu_profiles"
  | "edit_menu_profiles"
  | "delete_menu_profiles";
...
const ROLE_CAPS: Record<Role, Capability[]> = {
  ...
  waiter: ["view_waiter_orders", "manage_waiter_orders", "manage_tables", "view_orders"],
};
...
export function isPathAllowedForRole(role: Role | undefined, path: string): boolean {
  ...
  const PATH_CAPS: { pattern: RegExp; cap: Capability }[] = [
    ...
    { pattern: /^\/menu-profiles/, cap: "view_menu_profiles" },
  ];
