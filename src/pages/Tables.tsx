...
export default function Tables() {
  ...
  // Open the dedicated Table Order screen (POS-style) for a table card.
  // SPLIT parents themselves are not directly orderable — the user
  // should pick a section. Anything else (free/reserved/busy/section/
  // merged primary) opens the order screen for that table id.
  const openTableOrder = (t: RestaurantTable) => {
    if (t.status === "split" && !t.parentTableId) {
      toast.push("Pick a section (e.g. A or B) to take an order", "info");
      return;
    }
    navigate(`/tables/${t.id}/order`);
  };
          ...
          {filtered.map((t) => {
            ...
            return (
              ...
              >
                ...
                {/* Big number — clickable: opens the Table Order screen */}
                <button
                  type="button"
                  onClick={() => openTableOrder(t)}
                  title={`Open order for Table ${displayNumber(t)}`}
                  className="w-full flex items-end justify-between gap-1 mb-2 sm:mb-3 text-left hover:opacity-90 active:scale-[0.99] transition"
                >
                ...
                <div className="flex flex-wrap items-start justify-between gap-x-1 gap-y-2 border-t border-white/10 pt-2">
                  <div className="flex flex-wrap gap-1">
                    <button
                      onClick={() => openTableOrder(t)}
                      title="Open Order Screen"
                      className="flex flex-col items-center justify-center gap-0.5 px-1.5 py-1 rounded bg-white/15 hover:bg-white/25 text-white"
                    >
