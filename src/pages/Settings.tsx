...
import {
  getPrintScale, setPrintScale, getPrintFontScale, setPrintFontScale,
...
export default function Settings() {
  ...
  const [fontScale, setFontScaleState] = useState<number>(() => getPrintFontScale());
  const [previewType, setPreviewType] = useState<"bill" | "kot">("bill");

  // Persist print-size and font-scale immediately on change.
  useEffect(() => { setPrintScale(printScale); }, [printScale]);
  useEffect(() => { setPrintFontScale(fontScale); }, [fontScale]);

  const previewHtml = useMemo(
    () => buildPreviewHtml(form, previewType, { printScale, fontScale }),
    [form, previewType, printScale, fontScale],
  );

  const bumpFont = (delta: number) =>
    setFontScaleState((v) => {
      const next = Math.min(FONT_SCALE_BOUNDS.max, Math.max(FONT_SCALE_BOUNDS.min, +(v + delta).toFixed(2)));
      return next;
    });
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <Button
                    variant="secondary"
                    onClick={() => bumpFont(-FONT_SCALE_BOUNDS.step)}
                    disabled={fontScale <= FONT_SCALE_BOUNDS.min}
                  ...
                  <Button
                    variant="secondary"
                    onClick={() => bumpFont(FONT_SCALE_BOUNDS.step)}
                    disabled={fontScale >= FONT_SCALE_BOUNDS.max}
                    aria-label="Increase font size"
                  >
                    <Plus className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" onClick={() => setFontScaleState(1)}>Reset</Button>
                </div>
                <p className="text-xs text-neutral-500 mt-2">
                  Adjusts text size on both Bill and KOT print.
                </p>
              </div>
