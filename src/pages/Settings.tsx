      ...
      <Card>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <Button
            variant="danger"
            onClick={() => {
              if (!confirm("Close the business day right now?\n\nDashboard and Reports 'today' counters will reset immediately. Unpaid bills carry forward — no data is deleted.")) return;
              Store.closeBusinessDayNow(user ? { id: user.id, name: user.name } : undefined);
              toast.push("Business day closed. Today's counters reset.", "success");
            }}
          >
            <RotateCcw className="h-4 w-4" /> Close Business Day Now
          </Button>
          <span className="text-xs text-neutral-500">
            Use this to force an immediate day-close (e.g. if the automatic close didn't fire).
          </span>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            variant="primary"
            onClick={async () => {
              const { MENU_SPEC } = await import("../lib/menuSpec");
              if (
                !window.confirm(
                  "Sync the menu from the master list?\n\n• New categories/items will be created\n• Existing items will have prices/names/veg flag updated\n• Anything not in the master list will be DELETED\n\nMenu Profiles (festival menus) are NOT affected."
                )
              )
                return;
              const res = Store.syncMenuFromSpec(MENU_SPEC);
              if (user)
                Store.addAudit({
                  userId: user.id,
                  userName: user.name,
                  action: "SYNC_MENU_SPEC",
                  details: `Created ${res.catsCreated} cats / ${res.itemsCreated} items · Updated ${res.itemsUpdated} · Deleted ${res.catsDeleted} cats / ${res.itemsDeleted} items`,
                });
              toast.push(
                `Menu synced — +${res.itemsCreated} new, ~${res.itemsUpdated} updated, -${res.itemsDeleted} removed`,
                "success"
              );
            }}
          >
            <UploadCloud className="h-4 w-4" /> Sync Menu from Master List
          </Button>
        </div>
      </Card>
