...
export default function Waiter() {
  ...
  useEffect(() => {
    ...
    // Mark every hydrated item as already-printed so subsequent KOT
    // presses generate an Additional KOT containing only the items the
    // waiter newly adds to this busy table.
    setDraft(activeOrder.items.map((i) => markKotPrinted(i)));
    setNotes(activeOrder.notes ?? "");
    setAutoLoadedFor(activeOrder.id);
    toast.push(
      `Loaded existing order ${activeOrder.billNumber} — new items will print as Additional KOT`,
      "info"
    );
  ...
  const addItem = (it: { id: string; name: string; price: number }) => {
    setDraft((p) => {
      // Only merge into a line that has NOT been printed yet. Printed
      // lines are part of the existing order and must remain frozen so
      // the Additional KOT only contains the new portion.
  ...
  const upsertItems = (
    ...
    // Only the NEW (unprinted) draft lines should be sent to the order.
    // Hydrated/printed lines are already part of the active order and
    // must not be re-appended (which would double their quantity).
    const now = Date.now();
    const unprintedDraft = draft.filter((d) => d.kotPrinted !== true);
    const sourceLines = activeOrder ? unprintedDraft : draft;
    if (activeOrder && sourceLines.length === 0) {
      toast.push("No new items to add.", "info");
      return null;
    }
    const newItems: OrderItem[] = sourceLines.map((d) => ({
      ...d,
  ...
  const handleSaveOrder = () => {
    if (!validate()) return;
    if (activeOrder && draft.some((d) => d.kotPrinted !== true)) {
      doPrintKOT();
      return;
    }
  ...
  const doPrintKOT = () => {
    // Capture additional-KOT context BEFORE upsert mutates state.
    const newItemsSnapshot: OrderItem[] = draft
      .filter((d) => d.kotPrinted !== true)
      .map((d) => ({ ...d }));
    const wasBusyAdditional = !!activeOrder && newItemsSnapshot.length > 0;

    const order = upsertItems("confirmed", "WAITER_KOT_PRINT");
    if (!order) return;

    if (wasBusyAdditional && newItemsSnapshot.length > 0) {
      // Additional KOT — print ONLY the newly added items, keep the
      // same bill number, log to kotHistory so the kitchen and the
      // "Reprint Last Additional KOT" button can replay it.
      const kotRecord = {
        id: Store.uid("kot"),
        ts: Date.now(),
        kind: "additional" as const,
        itemIds: newItemsSnapshot.map((d) => d.id),
        waiterId: user?.id,
        waiterName: user?.name,
        acknowledged: false,
      };
      Store.updateOrder(order.id, {
        items: order.items.map((item) =>
          newItemsSnapshot.some((ni) => ni.id === item.id)
            ? markKotPrinted(item)
            : item
        ),
        kotHistory: [...(order.kotHistory ?? []), kotRecord],
      });
      printAdditionalKot({ ...order, waiterName: user?.name }, newItemsSnapshot, settings);
      toast.push(`Additional KOT printed for ${order.billNumber}`, "success");
      Store.addNotification({
        type: "new_order",
        message: `New items added to Table T${order.tableNumber ?? "—"}.`,
        orderId: order.id,
      });
    } else {
      // Initial KOT — full print as before.
      const kotRecord = {
        id: Store.uid("kot"),
        ts: Date.now(),
  ...
  // Reprint the most recent Additional KOT for the currently selected
  // table's active order.
  const handleReprintLastAdditional = () => {
    const ord = activeOrder;
    if (!ord) {
      toast.push("No active order on this table.", "info");
      return;
    }
    const last = [...(ord.kotHistory ?? [])].reverse().find((k) => k.kind === "additional");
    if (!last) {
      toast.push("No Additional KOT to reprint yet.", "info");
      return;
    }
    const subset = ord.items.filter((i) => last.itemIds.includes(i.id));
    if (subset.length === 0) {
      toast.push("Additional KOT items no longer on this order.", "error");
      return;
    }
    printAdditionalKot(ord, subset, settings);
    toast.push(`Reprinted last Additional KOT for ${ord.billNumber}`, "success");
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
      <div className="lg:col-span-7 flex flex-col lg:flex-row gap-4 min-w-0">
        <MenuCategorySidebar
      ...
      <div className="lg:col-span-5">
        <Card className="sticky top-20 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {activeOrder && (activeOrder.kotHistory ?? []).some((k) => k.kind === "additional") && (
              <Button variant="outline" onClick={handleReprintLastAdditional} className="col-span-2">
                <Printer className="h-4 w-4" /> Reprint Last Additional KOT
              </Button>
            )}
          </div>
        </Card>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 pt-2 sticky bottom-0 bg-white dark:bg-neutral-900 -mx-5 px-5 pb-1 border-t border-neutral-200 dark:border-neutral-800 mt-2">
            ...
            {activeOrder && (activeOrder.kotHistory ?? []).some((k) => k.kind === "additional") && (
              <Button
                variant="outline"
                onClick={() => { handleReprintLastAdditional(); setCartOpen(false); }}
