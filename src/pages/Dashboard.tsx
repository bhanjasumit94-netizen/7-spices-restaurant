...
import { getBusinessDayStart, formatBusinessDayRange } from "../lib/businessDay";
...
export default function Dashboard() {
  ...
  const settings = useStore("spices_settings", Store.getSettings);

  const stats = useMemo(() => {
    const now = new Date();
    const startOfDay = getBusinessDayStart(now.getTime(), settings);
  ...
  }, [orders, tables, inventory, settings]);
      ...
      >
        <div>
          ...
          <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
            Business day: {formatBusinessDayRange(Date.now(), settings)}
          </p>
        </div>
