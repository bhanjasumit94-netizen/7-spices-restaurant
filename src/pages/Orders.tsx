...
import { printBill, printAdditionalKot } from "../lib/printer";
...
export default function Orders() {
  ...
  const [pickerCat, setPickerCat] = useState<string>(
    () => localStorage.getItem("orders.pickerCat") || "all"
  );
  const [pickerSearch, setPickerSearch] = useState("");
  useEffect(() => {
    localStorage.setItem("orders.pickerCat", pickerCat);
  }, [pickerCat]);
  ...
  // REALTIME SYNC: when the underlying orders array updates (Additional KOT
  // print, items appended from Waiter/POS, payment posted from elsewhere),
  // refresh the open detail modal so it shows the new items, totals, and
  // status without requiring the user to close & reopen it.
  //
  // IMPORTANT: while the user is editing locally (adding items, changing
  // quantities, removing items) the local `detailOrder` is the in-progress
  // draft and is intentionally ahead of Firestore until "Save Changes"
  // persists it. We must NOT overwrite that draft from the snapshot, or
  // every add/remove/qty change disappears immediately. Only sync when
  // the DB record is strictly newer than what we have locally.
  useEffect(() => {
    if (!detailOrder) return;
    if (editingOrder) return; // user is mid-edit — never clobber local draft
    const fresh = orders.find((o) => o.id === detailOrder.id);
    if (!fresh) return;
    if ((fresh.updatedAt ?? 0) > (detailOrder.updatedAt ?? 0)) {
      console.log("[Orders] Firestore update received", {
        orderId: fresh.id,
        tableId: fresh.tableId,
        itemsCount: fresh.items.length,
        grandTotal: fresh.grandTotal,
        status: fresh.status,
      });
      setDetailOrder(fresh);
    }
  }, [orders, detailOrder, editingOrder]);
  ...
  useEffect(() => {
    ...
    // Always keep the Cash / UPI field in sync with the latest remaining
    // balance so item add/remove/qty changes are reflected immediately —
    // no need to press Save Payment to recalc the final amount.
  ...
  // Print KOT for ONLY newly-added items (kotPrinted === false). Does not
  // touch existing KOT printing, reprint, bill, or payment flows.
  const printNewItemsKOT = (o: Order) => {
    // Pending qty per line = quantity - kotPrintedQty. Legacy items without
    // kotPrintedQty fall back to the boolean flag (fully printed / not).
    const pending = (o.items || [])
      .map((i) => {
        const printedQty =
          i.kotPrintedQty ?? (i.kotPrinted || i.printed ? i.quantity : 0);
        const deltaQty = Math.max(0, i.quantity - printedQty);
        return { item: i, deltaQty };
      })
      .filter((p) => p.deltaQty > 0);
    console.log("[Orders] printNewItemsKOT pending count:", pending.length, "orderId:", o.id, "bill:", o.billNumber);
    if (pending.length === 0) {
      toast.push("No new items pending KOT", "info");
      return;
    }
    // Build synthetic OrderItem entries containing only the delta qty so the
    // kitchen ticket shows "Chicken Malai Tikka × 1" for a +1 bump on an
    // existing "× 2" line.
    const printItems = pending.map((p) => ({ ...p.item, quantity: p.deltaQty }));
    console.log("[Orders] printing new items:", printItems.map((i) => ({ id: i.id, name: i.name, qty: i.quantity })));
    printAdditionalKot(o, printItems, settings);
    const pendingIds = new Set(pending.map((p) => p.item.id));
    const nextItems = (o.items || []).map((i) =>
      pendingIds.has(i.id)
        ? { ...i, kotPrinted: true, printed: true, kotPrintedQty: i.quantity }
        : i,
    );
    const kotHistory = [
      ...(o.kotHistory || []),
      {
        ...
        itemIds: Array.from(pendingIds),
        waiterId: user?.id,
        waiterName: user?.name,
      },
    ];
    Store.updateOrder(o.id, { items: nextItems, kotHistory, updatedAt: Date.now() });
    console.log("[Orders] kotPrinted status after print:", nextItems.map((i) => ({ id: i.id, name: i.name, kotPrinted: i.kotPrinted, kotPrintedQty: i.kotPrintedQty })));
    toast.push(`Printed ${pending.length} new item(s) KOT`, "success");
    if (detailOrder?.id === o.id) {
      setDetailOrder({ ...o, items: nextItems, kotHistory, updatedAt: Date.now() });
    }
    if (user) {
      Store.addAudit({
        userId: user.id,
        userName: user.name,
        action: "PRINT_NEW_ITEMS_KOT",
        details: `${o.billNumber} • ${pending.length} new item(s)`,
      });
    }
  };
  ...
  const editIncQty = (itemId: string) => {
    if (!detailOrder) return;
    const before = detailOrder.items.length;
    const target = detailOrder.items.find((i) => i.id === itemId);
    if (!target) return;
    // Merge into the same line — the delta over kotPrintedQty is what the
    // "New Items KOT" will print, so the line stays as e.g. "Chicken Malai
    // Tikka × 2" instead of splitting into two separate rows.
    const nextItems = detailOrder.items.map((i) => {
      if (i.id !== itemId) return i;
      // Freeze how many units were already sent to the kitchen BEFORE we bump
      // the qty, so the delta (new − printed) is what "New Items KOT" prints.
      const printedQty =
        i.kotPrintedQty ?? (i.kotPrinted || i.printed ? i.quantity : 0);
      return {
        ...i,
        quantity: i.quantity + 1,
        kotPrintedQty: printedQty,
        kotPrinted: false,
        printed: false,
      };
    });
    const totals = recomputeEditedTotals({ ...detailOrder, items: nextItems });
    const balanceDue = Math.max(0, totals.grandTotal - (detailOrder.amountPaid ?? 0));
    console.log("[Orders] editIncQty", {
      orderId: detailOrder.id,
      tableId: detailOrder.tableId,
      itemId,
      itemsBefore: before,
      itemsAfter: nextItems.length,
  ...
  const editDecQty = (itemId: string) => {
    if (!detailOrder) return;
    const before = detailOrder.items.length;
    const nextItems = detailOrder.items
      .map((i) => {
        if (i.id !== itemId) return i;
        const nextQty = i.quantity - 1;
        // Clamp kotPrintedQty so we never claim more units were printed than
        // now exist on the line.
        const printedQty = i.kotPrintedQty ?? (i.kotPrinted || i.printed ? i.quantity : 0);
        const nextPrintedQty = Math.min(printedQty, Math.max(0, nextQty));
        return {
          ...i,
          quantity: nextQty,
          kotPrintedQty: nextPrintedQty,
          kotPrinted: nextPrintedQty >= nextQty && nextQty > 0,
          printed: nextPrintedQty >= nextQty && nextQty > 0,
        };
      })
      .filter((i) => i.quantity > 0);
    const totals = recomputeEditedTotals({ ...detailOrder, items: nextItems });
    const balanceDue = Math.max(0, totals.grandTotal - (detailOrder.amountPaid ?? 0));
    console.log("[Orders] editDecQty", {
      orderId: detailOrder.id,
      tableId: detailOrder.tableId,
      itemId,
      itemsBefore: before,
  ...
  const editRemove = (itemId: string) => {
    if (!detailOrder) return;
    const before = detailOrder.items.length;
    const nextItems = detailOrder.items.filter((i) => i.id !== itemId);
    const totals = recomputeEditedTotals({ ...detailOrder, items: nextItems });
    const balanceDue = Math.max(0, totals.grandTotal - (detailOrder.amountPaid ?? 0));
    console.log("[Orders] editRemove", {
      orderId: detailOrder.id,
      tableId: detailOrder.tableId,
      itemId,
      itemsBefore: before,
  ...
  const addMenuItemToOrder = (menuItemId: string) => {
    ...
    const totals = recomputeEditedTotals({ ...detailOrder, items: nextItems });
    const balanceDue = Math.max(0, totals.grandTotal - (detailOrder.amountPaid ?? 0));
    console.log("[Orders] addMenuItemToOrder", {
      orderId: detailOrder.id,
      tableId: detailOrder.tableId,
      menuItemId,
      itemsBefore: before,
  ...
  const decMenuItemQty = (menuItemId: string) => {
    if (!detailOrder) return;
    const existing = detailOrder.items.find(
      (i) => i.menuItemId === menuItemId || i.id === menuItemId
    );
    ...
    if (existing.quantity <= 1) {
      nextItems = detailOrder.items.filter(
        (i) => i.menuItemId !== menuItemId && i.id !== menuItemId
      );
    } else {
      nextItems = detailOrder.items.map((i) =>
        ...
        {detailOrder && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t">
              ...
              {/* New Items Pending KOT — only items added since the last KOT */}
              {(() => {
                const pendingCount = (detailOrder.items || []).filter((i) => {
                  const printedQty = i.kotPrintedQty ?? (i.kotPrinted || i.printed ? i.quantity : 0);
                  return i.quantity - printedQty > 0;
                }).length;
                console.log("[Orders] detail modal pending KOT count:", pendingCount, "orderId:", detailOrder.id);
                if (pendingCount === 0) return null;
                return (
                  <>
                    <Badge tone="warning">New Items Pending KOT ({pendingCount})</Badge>
                    <Button size="sm" variant="primary" onClick={() => printNewItemsKOT(detailOrder)}>
                      <Printer className="h-4 w-4" /> 🖨️ Print New Items KOT
                    </Button>
                  </>
                );
              })()}
...
// ────────────────────────────────────────────────────────────────────────────
// PickerBody — sidebar-based Add Menu Item picker used inside Edit Order.
// Left: scrollable category list with counts. Right: items of the selected
// category with search scoped to that category. Auto-scrolls the selected
// category into view when the popup opens.
// ────────────────────────────────────────────────────────────────────────────
...
function PickerBody({
  ...
  allItems: { id: string; name: string; price: number; categoryId: string; available: boolean; veg?: boolean }[];
  pickerCat: string;
  setPickerCat: (v: string) => void;
  pickerSearch: string;
  setPickerSearch: (v: string) => void;
  pickerItems: { id: string; name: string; price: number; categoryId: string; veg?: boolean }[];
  detailOrder: Order | null;
  addMenuItemToOrder: (id: string) => void;
  decMenuItemQty: (id: string) => void;
  onClose: () => void;
}) {
  const listRef = useRef<HTMLDivElement | null>(null);
  const activeBtnRef = useRef<HTMLButtonElement | null>(null);

  // Counts per category (available items only).
  ...
  const totalAvailable = useMemo(
    () => allItems.filter((i) => i.available).length,
    [allItems]
  );

  // Auto-scroll selected category into view on mount / when it changes.
  useEffect(() => {
  ...
  const activeCatName =
    pickerCat === "all"
      ? "All Items"
      : allCategories.find((c) => c.id === pickerCat)?.name ?? "All Items";

  return (
    <div className="flex flex-col md:flex-row gap-3 h-[80vh] max-h-[80vh] min-h-0">
      {/* LEFT — Categories */}
      <aside
        ref={listRef}
        className="md:w-[30%] md:min-w-[200px] md:max-w-[280px] shrink-0 border border-neutral-200 dark:border-neutral-800 rounded-lg overflow-y-auto bg-neutral-50 dark:bg-neutral-900/40"
      >
        <div className="sticky top-0 z-10 px-3 py-2 bg-neutral-100/90 dark:bg-neutral-800/90 backdrop-blur border-b border-neutral-200 dark:border-neutral-800 text-[11px] font-semibold uppercase tracking-wider text-neutral-500">
          Categories
        </div>
        <div className="p-2 space-y-0.5">
          <button
            ref={pickerCat === "all" ? activeBtnRef : null}
            onClick={() => setPickerCat("all")}
            className={cn(
              "w-full flex items-center justify-between gap-2 px-3 py-2 rounded-md text-left text-sm transition",
              pickerCat === "all"
      ...
      <div className="flex-1 min-w-0 flex flex-col min-h-0">
        <div className="flex items-center gap-2 mb-2">
          ...
          <Badge tone="neutral">{pickerItems.length} items</Badge>
        </div>

        <div className="flex-1 min-h-0 overflow-y-auto border border-neutral-200 dark:border-neutral-800 rounded-lg p-2 bg-white dark:bg-neutral-900">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {pickerItems.map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className="border border-neutral-200 dark:border-neutral-800 rounded-lg p-3 bg-white dark:bg-neutral-900 hover:border-gold-400/60 transition flex flex-col"
                  >
                    {/* Row 1: Name + Price */}
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <p className="text-sm font-bold text-gold-600 dark:text-gold-400 shrink-0 whitespace-nowrap mt-0.5">
                        {inr(m.price)}
                      </p>
                    </div>

                    {/* Row 2: Qty controls or Add button — right aligned */}
                    <div className="mt-auto pt-2 flex justify-end">
                      {existing ? (
                        <div className="flex items-center gap-1.5">
                          <button
                            onClick={() => decMenuItemQty(m.id)}
                            className="h-[34px] w-[34px] rounded-lg border border-neutral-300 dark:border-neutral-700 flex items-center justify-center hover:bg-neutral-100 dark:hover:bg-neutral-800 transition"
                          ...
                          <button
                            ...
                            className="h-[34px] w-[34px] rounded-lg bg-gold-gradient text-white flex items-center justify-center shadow-sm hover:opacity-90 transition"
                        ...
                        <button
                          onClick={() => addMenuItemToOrder(m.id)}
                          className="bg-gold-gradient text-white font-semibold flex items-center justify-center gap-1 shadow-sm hover:opacity-90 transition"
                          style={{
                            width: 90,
                            height: 34,
                            fontSize: 13,
        ...
        <div className="flex justify-end gap-2 pt-2 mt-2 border-t border-neutral-200 dark:border-neutral-800">
