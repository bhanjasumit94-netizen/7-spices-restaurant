...
export default function Orders() {
  ...
  const printNewItemsKOT = (o: Order) => {
    const pending = (o.items || []).filter((i) => i.kotPrinted !== true && i.printed !== true);
    ...
    console.log("[Orders] printing new items:", pending.map((i) => ({ id: i.id, name: i.name, qty: i.quantity })));
    printAdditionalKot(o, pending, settings);
    const pendingIds = new Set(pending.map((i) => i.id));
    const nextItems = (o.items || []).map((i) =>
      pendingIds.has(i.id) ? { ...i, kotPrinted: true, printed: true } : i,
    );
    const kotHistory = [
      ...(o.kotHistory || []),
      {
    ...
    console.log("[Orders] kotPrinted status after print:", nextItems.map((i) => ({ id: i.id, name: i.name, kotPrinted: i.kotPrinted })));
  ...
  const editIncQty = (itemId: string) => {
    ...
    if (!target) return;
    const alreadyPrinted = target.kotPrinted === true || target.printed === true;
    let nextItems;
    if (alreadyPrinted) {
      // Split: keep printed line as-is, add a fresh unprinted +1 line so the
      // "New Items KOT" only prints the delta and the button appears.
      const existingDelta = detailOrder.items.find(
        (i) =>
      ...
      if (existingDelta) {
        nextItems = detailOrder.items.map((i) =>
          i.id === existingDelta.id ? { ...i, quantity: i.quantity + 1 } : i,
        );
      } else {
        nextItems = [
          ...detailOrder.items,
          {
            ...
            price: target.price,
            quantity: 1,
            source: target.source ?? detailOrder.source,
            kotPrinted: false,
            addedAt: Date.now(),
          },
        ];
      ...
      nextItems = detailOrder.items.map((i) =>
        i.id === itemId
          ? { ...i, quantity: i.quantity + 1, kotPrinted: false }
          : i,
      );
    }
  ...
  const editDecQty = (itemId: string) => {
    ...
    const nextItems = detailOrder.items
      .map((i) =>
        i.id === itemId
          ? { ...i, quantity: i.quantity - 1, kotPrinted: false, printed: false }
          : i,
      )
        ...
        {detailOrder && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t">
              ...
              {(() => {
                const pendingCount = (detailOrder.items || []).filter(
                  (i) => i.kotPrinted !== true && i.printed !== true,
                ).length;
