...
export default function QRMenu() {
  ...
  // Resolve the secure token to a table id. If invalid -> render the
  // "Invalid QR" screen (still public, no login redirect).
  // We depend on `tables` so that the tableId refreshes once seeding completes.
  const tableId = useMemo(() => resolveToken(token), [token, tables]);
  const table = useMemo(() => tables.find((t) => t.id === tableId), [tables, tableId]);
  const invalid = !!token && !tableId;
